...
def init_index_info():
    ...
    try:
        for index_name, index_info in index_mapping.items():
            # 检查指数是否已存在
            existing = (
                db.query(models.IndexInfo).filter_by(index_name=index_name).first()
            )
            if existing:
...
def sync_index_history(
    index_name: str = None, start_date: str = None, end_date: str = None
):
    """同步指数历史数据到数据库
    ...
    try:
        if index_name:
            index_list = (
                db.query(models.IndexInfo).filter_by(index_name=index_name).all()
            )
            if not index_list:
        ...
        for index_info in index_list:
            ...
            # 从东方财富API获取历史数据
            history_data = scraper.get_index_history(
                index_info.index_name, start_date, end_date
            )
            if not history_data:
            ...
            logger.info(
                f"成功获取 {index_info.index_name} {len(history_data)} 条历史数据"
            )
            ...
            for data in history_data:
                ...
                if (saved_count + updated_count) % 100 == 0:
                    db.commit()
                    logger.info(
                        f"已处理 {index_info.index_name} {saved_count + updated_count} 条数据"
                    )
            ...
            logger.info(
                f"{index_info.index_name} 同步完成: 新增 {saved_count} 条，更新 {updated_count} 条，跳过 {skipped_count} 条"
            )
...
def export_index_history_to_csv(index_name: str = None, file_path: str = None):
    ...
    try:
        ...
        for index_info in index_list:
            ...
            history_data = (
                db.query(models.IndexHistory)
                .filter(models.IndexHistory.index_id == index_info.id)
                .order_by(models.IndexHistory.trade_date)
                .all()
            )
