...
class FundBasic(Base):
    ...
    fund_code = Column(
        String(20), unique=True, index=True, nullable=False, comment="fund_code"
    )
    ...
    company_id = Column(
        Integer,
        ForeignKey("fund_company.id"),
        nullable=True,
        index=True,
        comment="company_id",
    )
    company_name = Column(
        String(100), comment="company_name"
    ...
    daily_growth = Column(Float, comment="日增长率%")
    created_at = Column(
...
class FundCompany(Base):
    ...
    id = Column(Integer, primary_key=True, index=True, autoincrement=True)
    company_code = Column(
        String(20), unique=True, index=True, nullable=False, comment="company_code"
    )
    company_name = Column(
        String(100), index=True, nullable=False, comment="company_name"
    )
    short_name = Column(String(50), comment="short_name")
...
class User(Base):
    ...
    email = Column(
        String(100), unique=True, index=True, nullable=False, comment="email"
    )
    password_hash = Column(String(255), nullable=False, comment="password_hash")
    role = Column(
        Enum(UserRole), default=UserRole.USER, nullable=False, comment="user_role"
    )
...
class UserFavoriteFund(Base):
    ...
    fund_id = Column(
        Integer, ForeignKey("fund_basic.id"), nullable=False, comment="fund_id"
    )
    ...
    # Unique constraint
    __table_args__ = (UniqueConstraint("user_id", "fund_id", name="_user_fund_uc"),)
...
class IndexInfo(Base):
    ...
    id = Column(Integer, primary_key=True, index=True, autoincrement=True)
    index_name = Column(
        String(50), unique=True, index=True, nullable=False, comment="指数名称"
    )
    index_code = Column(
        String(20), unique=True, index=True, nullable=False, comment="指数代码"
    )
    secid = Column(
        String(20), unique=True, index=True, nullable=False, comment="东方财富secid"
    )
