...
class ScrapeService:
    ...
    def update_fund_growth(
        self, source: DataSource, fund_code_list: List[str] = None
    ) -> Dict[str, Any]:
        """更新基金历史涨幅数据
        ...
        if not fund_code_list:
            ...
            fund_code_list = [
                fund.fund_code for fund in self.db.query(models.FundBasic).all()
            ]
        ...
        for fund_code in fund_code_list:
            try:
                # 获取基金
                fund = (
                    self.db.query(models.FundBasic)
                    .filter(models.FundBasic.fund_code == fund_code)
                    .first()
                )
                ...
                # 查找现有涨幅数据
                existing_growth = (
                    self.db.query(models.FundGrowth)
                    .filter(
                ...
                if existing_growth:
                    ...
                    for growth_item in growth_data:
                        if growth_item["growth_type"] == "近1日":
                            ...
                            existing_growth.quarterly_growth = growth_item[
                                "growth_value"
                            ]
                    ...
                    new_growth = models.FundGrowth(
                        fund_id=fund.id, update_date=current_date
                    )
                ...
                self.logger.error(
                    f"更新基金历史涨幅数据失败，基金代码: {fund_code}，错误: {str(e)}"
                )
                self.db.rollback()
                failed_count += 1

        self.logger.info(
            f"基金历史涨幅数据更新完成，数据源: {source}，总数量: {total_count}，成功: {success_count}，失败: {failed_count}"
        )
    ...
    def sync_fund_company_relation(self, source: DataSource) -> Dict[str, Any]:
        ...
        for company_data in company_list:
            try:
                # 检查公司是否已存在
                existing_company = (
                    self.db.query(models.FundCompany)
                    .filter(
                ...
                if existing_company:
                    ...
                    if (
                        "established_date" in company_data
                        and company_data["established_date"]
                    ):
                        existing_company.establish_date = company_data[
                            "established_date"
                        ]
                else:
                    # 创建新公司
                    new_company = models.FundCompany(
                        ...
                        registered_capital=(
                            float(company_data.get("asset_scale", 0))
                            if company_data.get("asset_scale")
                            else None
                        ),
                ...
                self.logger.error(
                    f"处理基金公司数据失败，公司代码: {company_data['company_code']}, 错误: {str(e)}"
                )
                self.db.rollback()
                continue

        self.logger.info(
            f"基金公司数据导入完成，成功: {company_success}, 总数量: {len(company_list)}"
        )
        ...
        for relation in fund_relations:
            try:
                ...
                company = (
                    self.db.query(models.FundCompany)
                    .filter(models.FundCompany.company_name == company_name)
                    .first()
                )
                ...
                if fund.company_id != company.id:
                    ...
                    relation_success += 1
                    self.logger.info(
                        f"更新基金关联成功，基金代码: {fund_code}, 公司名称: {company_name}"
                    )
                else:
                    self.logger.debug(
                        f"基金关联已存在，基金代码: {fund_code}, 公司名称: {company_name}"
                    )
                    relation_success += 1
            except Exception as e:
                self.logger.error(
                    f"处理基金关联关系失败，基金代码: {relation.get('fund_code', '未知')}, 错误: {str(e)}"
                )
                self.db.rollback()
                continue

        self.logger.info(
            f"基金与公司关联关系同步完成，成功: {relation_success}, 总数量: {len(fund_relations)}"
        )
        ...
        return {
            ...
            "relation_sync": {
                "success": relation_success,
                "total": len(fund_relations),
            },
        }

    def update_fund_rank(
        self, source: DataSource, max_pages: int = None
    ) -> Dict[str, Any]:
        """更新基金排行数据
        ...
        """
        self.logger.info(
            f"开始更新基金排行数据，数据源: {source}, max_pages: {max_pages}"
        )
        ...
        for rank, fund_data in enumerate(fund_rank_data, 1):
            try:
                ...
                fund = (
                    self.db.query(models.FundBasic)
                    .filter(models.FundBasic.fund_code == fund_data["fund_code"])
                    .first()
                )
                ...
                if not fund:
                    # 基金不存在，创建新记录
                    self.logger.info(
                        f"基金不存在，创建新基金，基金代码: {fund_data['fund_code']}"
                    )
                    ...
                    if fund_data.get("launch_date"):
                        try:
                            new_fund.launch_date = datetime.strptime(
                                fund_data["launch_date"], "%Y-%m-%d"
                            ).date()
                        except ValueError:
                            pass
                    ...
                    if fund_data.get("daily_growth") is not None:
                        fund.daily_growth = fund_data.get("daily_growth")
                    # 更新成立日期
                    if fund_data.get("launch_date"):
                        try:
                ...
                if existing_rank:
                    ...
                    existing_rank.rank_type = (
                        "daily_rank"  # 默认日排行，可根据实际情况调整
                    )
                    ...
                    existing_rank.since_launch_growth = fund_data.get(
                        "since_launch_growth"
                    )
                ...
                success_count += 1
                self.logger.info(
                    f"更新基金排行数据成功，基金代码: {fund_data['fund_code']}"
                )

            except Exception as e:
                self.logger.error(
                    f"处理基金排行数据失败，基金代码: {fund_data['fund_code']}，错误: {str(e)}"
                )
                self.db.rollback()
                failed_count += 1

        self.logger.info(
            f"基金排行数据更新完成，数据源: {source}，总数量: {total_count}，成功: {success_count}，失败: {failed_count}"
        )
        ...
        for fund_data in fund_list:
            try:
                ...
                if existing_fund:
                    # 基金已存在，不更新，直接跳过
                    self.logger.info(
                        f"基金已存在，跳过，基金代码: {fund_data['fund_code']}"
                    )
                    continue
                else:
                    # 创建新基金
                ...
                self.logger.error(
                    f"处理基金数据失败，基金代码: {fund_data['fund_code']}，错误: {str(e)}"
                )
                self.db.rollback()

        self.logger.info(
            f"基金列表导入完成，数据源: {source}，总数量: {total_count}，新增: {added_count}，更新: {updated_count}"
        )
    ...
    def import_fund_company_list(self, source: DataSource) -> Dict[str, Any]:
        ...
        for company_data in company_list:
            try:
                ...
                if existing_company:
                    # 公司已存在，不更新，直接跳过
                    self.logger.info(
                        f"公司已存在，跳过，公司代码: {company_data['company_code']}"
                    )
                    continue
                else:
                    # 创建新公司
                    new_company = models.FundCompany(
                        ...
                        short_name=company_data.get(
                            "short_name", company_data["company_name"]
                        ),  # 使用数据中的简称
        ...
        self.logger.info(
            f"基金公司列表导入完成，数据源: {source}，总数量: {total_count}，新增: {added_count}，更新: {updated_count}"
        )
    ...
    def create_scrape_all_funds_task(
        self, source: DataSource, data_type: DataType
    ) -> str:
        """创建采集所有基金的任务
    ...
    def create_scrape_task(
        self, source: DataSource, data_type: DataType, fund_code_list: List[str]
    ) -> str:
        """创建采集任务
        ...
        for fund_code in fund_code_list:
            task_item = ScrapeTaskItem(
                task_id=task.id, fund_code=fund_code, status="pending"
            )
        ...
        logger.info(
            f"创建采集任务成功，任务ID: {task_id}，数据源: {source}，数据类型: {data_type}，基金数量: {len(fund_code_list)}"
        )
        return task_id
    ...
    def run_scrape_task(self, task_id: str) -> Dict[str, Any]:
        ...
        # 查询任务项
        task_items = (
            self.db.query(ScrapeTaskItem)
            .filter(ScrapeTaskItem.task_id == task.id)
            .all()
        )
        ...
        try:
            raw_data_list = scraper.run(
                fund_code_list=fund_code_list, data_type=task.data_type
            )
                ...
                try:
                    ...
                    task_item = next(
                        (
                            ...
                            if item.fund_code == raw_data.fund_code
                        ),
                        None,
                    )
                    if task_item:
                    ...
                    logger.error(
                        f"保存原始数据失败，基金代码: {raw_data.fund_code}，错误: {str(e)}"
                    )
                    error_count += 1
            ...
            logger.info(
                f"采集任务完成，任务ID: {task_id}，成功: {success_count}，失败: {error_count}"
            )
            return {
                "status": "success",
                "message": "采集任务完成",
    ...
    def _save_raw_data(self, raw_data: Any) -> None:
        ...
        logger.info(
            f"保存原始数据成功，基金代码: {raw_data.fund_code}，数据类型: {raw_data.data_type}"
        )
    ...
    def get_scrape_history(
        self, page: int = 1, page_size: int = 10, **filters
    ) -> Dict[str, Any]:
        """获取采集历史记录
